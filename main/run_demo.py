...
def get_joint_setting(mesh_model, joint_category='coco'):
    ...
    # Tạo dictionary mới loại bỏ các key có chứa 'total_ops' và 'total_params'
    clean_state_dict = {
...
def optimize_cam_param(project_net, model, joint_input, proj_target_joint_img, img_features, bbox1, joint_regressor):
    ...
    # estimate mesh, pose
    model.eval()
    _, _, _, _,pred_mesh, _ = model(joint_img, img_feat)
...
def main(args):
    ...
    if args.img_file != 'None':
        image_folder = args.img_file
        video_file = args.img_file
    ...
    for person_id in tqdm(list(tracking_results.keys())):
        ...
        for idx, frame_id in enumerate(frames):
            ...
            if args.img_file != 'None':
                img_path = osp.join(image_folder, 'image_'+str(frame_id).zfill(5) + '.jpg')
            persons = []
            person_info = {}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--img_file', type=str, default='None', help='input image sequence path')
