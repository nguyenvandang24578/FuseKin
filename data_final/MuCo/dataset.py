...
class MuCo(torch.utils.data.Dataset):
    def __init__(self, transform, data_split):
        ...
        self.img_dir = osp.join(cfg.DATASET.jotr_data_root, 'MuCo')
        self.annot_path = osp.join(cfg.DATASET.jotr_data_root, 'MuCo', 'MuCo-3DHP.json')
        self.smpl_param_path = osp.join(cfg.DATASET.jotr_data_root, 'MuCo', 'smpl_param.json')
        ...
        # H36M joint set
        self.h36m_joint_regressor = np.load(osp.join(cfg.DATASET.jotr_data_root, 'Human36M', 'J_regressor_h36m_correct.npy')) # use h36m joint regrssor (only use subset from original muco joint set)
