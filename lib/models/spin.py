...
class RegressorSpin(nn.Module):
    def __init__(self, smpl_mean_params=SMPL_MEAN_PARAMS):
        super(RegressorSpin, self).__init__()
    ...
    def forward(self, x, init_pose=None, init_shape=None, init_cam=None, n_iter=3, is_train=False, J_regressor=None):
        batch_size = x.shape[0]
        seqlen = x.shape[1]
        x = x.reshape(batch_size * seqlen, 2048)

        if init_pose is None:
            init_pose = self.init_pose.expand(batch_size*seqlen, -1)
        if init_shape is None:
            init_shape = self.init_shape.expand(batch_size*seqlen, -1)
        if init_cam is None:
            init_cam = self.init_cam.expand(batch_size*seqlen, -1)
        ...
        pred_rotmat = rot6d_to_rotmat(pred_pose).view(batch_size*seqlen, 24, 3, 3)
        ...
        pose = rotation_matrix_to_angle_axis(pred_rotmat.reshape(-1, 3, 3)).reshape(batch_size, seqlen, 72)
        pred_cam = pred_cam.reshape(batch_size, seqlen,-1)
        pred_shape = pred_shape.reshape(batch_size, seqlen,-1)
        pred_vertices = pred_vertices.reshape(batch_size, seqlen,-1, 3)

        output = [{
            'theta'  : torch.cat([pred_cam, pose, pred_shape], dim=2),
            'verts'  : pred_vertices,
            'kp_2d'  : pred_keypoints_2d,
            'kp_3d'  : pred_joints,
            'rotmat' : pred_rotmat
